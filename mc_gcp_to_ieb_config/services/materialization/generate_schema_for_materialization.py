...
from mc_gcp_to_ieb_config.services.materialization.iedm_to_bigquery import (
    read_json,
    get_iedm_fields,
    get_bigquery_fields,
)
...
def write_bigquery_schema_json_to_airflow_directory(bigquery_fields: dict, original_iedm_path: str):
...
def get_airflow_directory_to_write_schema(original_iedm_path: str):
    return f"{get_airflow_path()}/dags/core/domain_event_materializer/schemas/{get_airflow_relative_directory_to_write_schema(original_iedm_path)}"
...
def collect_table_config(env, swimlane_name, stream, materialization_config, iedm_json):
    events_table_id = get_events_table_id(
        env,
        swimlane_name,
        stream.get("level_0"),
        stream.get("level_1"),
...
def airflow_schema_sync():
    ...
    validate_config(check_airflow_and_iedm_paths_present=True)
